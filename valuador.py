...
/* UI Asistente Gemini Investigación */
.copilot-prompt-card {
    background: #0d111a;
...
.copilot-pill:hover { background: rgba(212, 163, 75, 0.15); border-color: #d4a34b; color: #fbbf24; }
...
def ia_generar_respuesta_copilot(prompt_usuario, contexto_mercado=""):
    """Motor del Asistente Financiero con memoria de mercado y portafolio."""
    if HAS_GEMINI_LIB and GEMINI_KEY:
        try:
            model = genai.GenerativeModel("gemini-1.5-flash")
            full_prompt = f"""
            Actúa como un Senior Wealth Advisor y Portfolio Manager institucional de Wall Street.
            Eres el asistente de investigación de cabecera de Facu.
            
            Contexto de mercado y datos operativos disponibles:
            {contexto_mercado}
            
            Pregunta o instrucción de Facu:
            {prompt_usuario}
            
            Instrucciones de formato y tono:
            - Sé directo, técnico, analítico y profesional.
            - Utiliza métricas concretas (ROE, P/E, Deuda Neta/EBITDA, flujos de caja y covarianzas).
            - Ofrece recomendaciones taxativas y estructuradas con bullets o subtítulos limpios.
            - Evita introducciones vacías o disculpas robóticas.
            """
            res = model.generate_content(full_prompt)
            if res and res.text: return res.text.strip()
        except Exception as e:
            return f"Error en conexión AI: {e}"
            
    return (
        "**Diagnóstico Estratégico Base:** Para optimizar la exposición actual se sugiere mantener activos defensivos "
        "(XOM, KO) como estabilizadores de flujo por dividendos, complementando con activos de beta alto (NVDA, AMD) "
...
# Ponderaciones relativas aproximadas de Capitalización (para el Treemap de Finviz)
MARKET_CAPS = {
...
# ==============================================================================
# 2. ANÁLISIS & GOOGLE FINANCE STYLE COMPARATOR
# ==============================================================================
elif menu == "🔍 ANÁLISIS & GOOGLE COMPARATOR":
    ...
    # Auto-sugerencia inteligente de pares del mismo sector (Estilo Google Finance)
    ...
    # Si cambió el activo, actualizar los pares sugeridos por defecto
    ...
    peers_seleccionados = col_sel2.multiselect(
        f"Comparar con competidores de {sector_activo} (Sugeridos automáticamente):",
    ...
    tab_gfinance, tab_fund, tab_tech, tab_dcf = st.tabs([
        "📈 Comparador Relativo (Google Finance)", 
        "📊 Balances & Ratios", 
        "📈 Técnico (DMI)", 
        "🎯 DCF & Reverse DCF"
    ])

    # --- PESTAÑA GOOGLE FINANCE COMPARATOR ---
    with tab_gfinance:
        st.markdown(f"### 📊 Gráfico de Rendimiento Comparativo Relativo (Base 0%)")
        tf_col, _ = st.columns([2, 3])
        tf_sel = tf_col.radio("Ventana Temporal:", ["1M", "3M", "6M", "YTD", "1Y"], horizontal=True)
        ...
        if not df_comp.empty:
            ...
            fig_g = px.line(
                df_norm,
                labels={"value": "Variación (%)", "index": "Fecha"},
            ...
            # Tabla idéntica a Google Finance
            filas_gf = []
            for tk in tickers_comparativa:
                p_act = float(df_sub[tk].iloc[-1])
                p_prev = float(df_sub[tk].iloc[-2]) if len(df_sub) >= 2 else p_act
            ...
            tabla_gf_html = f"""<div class='table-viewport'><table class='terminal-table'>
                <thead><tr>
                    <th>Símbolo / Compañía</th><th>Precio USD</th><th>Cambio ($)</th><th>% Cambio (Hoy)</th><th>Cierre Anterior</th>
                </tr></thead>
                <tbody>{"".join(filas_gf)}</tbody>
            </table></div>"""
            st.markdown(tabla_gf_html, unsafe_allow_html=True)
    ...
    with tab_fund:
        ...
        matriz_html = f"<div class='table-viewport'><table class='terminal-table'><thead><tr><th>Ticker</th><th>Razón Social</th><th>P/E</th><th>EV/EBITDA</th><th>Deuda</th><th>Liquidez</th><th>Margen</th><th>ROE</th></tr></thead><tbody>{''.join(filas_peers)}</tbody></table></div>"
    ...
    with tab_dcf:
        ...
        st.markdown(f"<div class='terminal-card green-card'><b>DCF Fair Value:</b> ${dcf_mediano:.2f} USD | <b>Precio Mercado:</b> ${precio_mkt:.2f} USD | <b>Margen de Seguridad:</b> {((dcf_mediano - precio_mkt)/dcf_mediano)*100:+.1f}%</div>", unsafe_allow_html=True)
        tesis_macro = ia_tesis_macro_coyuntura(t_obj, d_fund["Nombre"], precio_mkt, d_fund["PE"], d_fund["ROE"], d_fund["DEUDA"], d_fund["MARGEN"], dcf_mediano)
        st.markdown(f"<div class='terminal-card blue-card'><b>Tesis de Coyuntura Macro:</b><br>{tesis_macro}</div>", unsafe_allow_html=True)

# ==============================================================================
# 3. PORTAFOLIO Y MODELOS (CON AUDITORÍA PDF Y BENCHMARK REAL)
# ==============================================================================
...
# ==============================================================================
    ...
    with col_q1:
        if st.button(f"🔍 {p_sug1}"):
            st.session_state.copilot_prompt_trigger = p_sug1
    with col_q2:
        if st.button(f"🚀 {p_sug2}"):
            st.session_state.copilot_prompt_trigger = p_sug2
    with col_q3:
    ...
    # Campo de Entrada de Pregunta Libre

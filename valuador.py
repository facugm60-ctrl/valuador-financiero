...
# ------------------------------------------------------------------------------
# DISFRAZ ANTI-BLOQUEO MEJORADO PARA YAHOO FINANCE & FINVIZ
# ------------------------------------------------------------------------------
...
def obtener_fundamental_completo(symbol):
    try:
        ...
        # Validamos si Yahoo mandó data vacía (P/E == 0)
        ...
        if not inf or yahoo_pe == 0.0:
            ...
            return {
                ...
                "MARGEN": fv_data.get("MARGEN", 0.0), "ROE": fv_data.get("ROE", 0.0), "RAW": inf, "Status": "Finviz" if fv_data else "Blocked"
            }
        ...
        return {
            ...
            "RAW": inf, "Status": "Yahoo"
        }
    except:
        return {"Ticker": symbol, "Nombre": symbol, "Precio": 50.0, "PE": 0.0, "EV": 0.0, "DEUDA": 0.0, "LIQUIDEZ": 0.0, "MARGEN": 0.0, "ROE": 0.0, "RAW": {}, "Status": "Error"}
...
# ------------------------------------------------------------------------------
# PESTAÑA DASHBOARD Y WATCHLIST (RESTAURADA)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# PESTAÑA ANÁLISIS INTEGRAL (FUSIÓN PERFECTA)
# ------------------------------------------------------------------------------
    ...
    if st.button("🔥 Correr Análisis"):
        with st.spinner(f"Sincronizando balances e historial para {t_obj}..."):
            ...
            # Verificación de bloqueo: Si todas las PE de los competidores son 0, Cloudflare bloqueó la IP.
            if all(d["PE"] == 0.0 for d in dataset):
                st.markdown("""<div style='background-color: rgba(231, 76, 60, 0.15); padding: 12px; border-left: 4px solid #e74c3c; font-size: 13px; color: #f87171;'><strong>⚠️ ALERTA DE BLOQUEO DE API:</strong> Las plataformas públicas (Yahoo Finance y Finviz) detectaron el servidor en la nube y bloquearon temporalmente la descarga de los ratios fundamentales. Por eso la matriz se muestra en 0. El Análisis Técnico y el Montecarlo funcionarán normalmente basados en precio.</div><br>""", unsafe_allow_html=True)
            ...
            if not serie_mc.empty:
                ...
                with tab_fund:
                    ...
                    if not desc_raw:
                        desc_final = FALLBACK_SUMMARIES.get(t_obj, f"{t_obj} se encuentra en la base de datos. (Descripción omitida por bloqueo temporal).")
                    else:
                        if HAS_TRANSLATOR:
                    ...
                    with col_caja:
                        ...
                        try:
                            ...
                            else: st.warning("Datos trimestrales temporalmente inaccesibles.")
                ...
                # --- SUB-PESTAÑA 4: MONTECARLO PRECIO (CORREGIDO SIN DRIFT) ---
                with tab_mc:
                    st.markdown("### 🎲 La Máquina del Tiempo (Caminata Aleatoria)")
                    st.markdown("<div class='agent-box'><b>Mejora Matemática:</b> Se eliminó la inercia (sesgo alcista infinito). Ahora el simulador se basa puramente en la volatilidad real, permitiendo ver escenarios de caída lógicos.</div>", unsafe_allow_html=True)
            ...
            else: st.error("No se pudieron recopilar series de tiempo para este ticker.")

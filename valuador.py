...
# ==============================================================================
FALLBACK_SUMMARIES = {
    "VIST": "Vista Energy es una compañía independiente de petróleo y gas, enfocada principalmente en la exploración y producción de Vaca Muerta, Argentina.",
    "YPF": "YPF Sociedad Anónima es la principal empresa energética de Argentina, dedicada a la exploración, producción, refinación y venta.",
    "XOM": "Exxon Mobil Corporation es uno de los gigantes energéticos más grandes del mundo.",
    ...
    "MSFT": "Microsoft Corporation es un líder global en software y computación en la nube (Azure).",
    "NVDA": "NVIDIA Corporation es el líder indiscutido en el diseño de unidades de procesamiento gráfico (GPUs).",
    "KO": "The Coca-Cola Company es la empresa de bebidas no alcohólicas más grande del planeta."
}
...
EXPLICACIONES_TECNICAS = {
    ...
    "DEUDA": "<b>Deuda / EBITDA:</b><br>Compara su deuda total con lo que genera en un año.",
    "LIQUIDEZ": "<b>Liquidez Corriente:</b><br>Efectivo y activos rápidos disponibles para pagar deudas de corto plazo.",
...
def obtener_fundamental_completo(symbol):
    try:
        tk_Bolsa = symbol + ".BA" if symbol in ["ALUA","BBAR","BMA","CEPU","COME","CRES","EDN","GGAL","LOMA","MIRG","PAMP","TECO2","TGNO4","TGSU2","TRAN","TXAR","YPF","BYMA","VALO","SUPV","BHIP","INVJ","LEDE"] else symbol
...
# ------------------------------------------------------------------------------
# PESTAÑA DASHBOARD Y WATCHLIST (RESTAURADO EXACTO)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# PESTAÑA ANÁLISIS INTEGRAL (RESTAURADO CON TÉCNICO INTACTO)
# ------------------------------------------------------------------------------
            ...
            if not serie_mc.empty:
                ...
                with tab_fund:
                    ...
                    with col_caja:
                        ...
                        try:
                            tk_ticker = yf.Ticker(t_obj + ".BA" if t_obj in ["ALUA","BBAR","BMA","CEPU","COME","CRES","EDN","GGAL","LOMA","MIRG","PAMP","TECO2","TGNO4","TGSU2","TRAN","TXAR","YPF","BYMA","VALO","SUPV"] else t_obj, session=yf_session)
                            ...
                            else: st.warning("Datos bloqueados en la nube.")
                ...
                # --- SUB-PESTAÑA 2: TÉCNICO (DMI) RESTAURADO EXACTO ---
                with tab_tech:
                ...
                # --- SUB-PESTAÑA 3: MONTECARLO CON CORRECCIÓN ---
                with tab_mc:
                    ...
                    Es un modelo estadístico puro que funciona mirando por el espejo retrovisor y estresa puramente la volatilidad ($\sigma$) del activo para evaluar riesgos reales de caída.<br><br>
                    </div>
                    """, unsafe_allow_html=True)
                    
                    ret = serie_mc.pct_change().dropna()
                    mu, sigma, p_b = 0.0, ret.std(), serie_mc.iloc[-1] # CORRECCIÓN DEL SESGO
                    c1, c2 = st.columns(2)
                    sims = 10000
                ...
                st.error("No se pudieron recopilar series de tiempo. Streamlit Cloud está bloqueado por Yahoo Finance. Corre el archivo app.py en tu entorno local.")

# ------------------------------------------------------------------------------
# PESTAÑA PORTAFOLIO Y MODELOS FACTORIALES (P&L RESTAURADO)
# ------------------------------------------------------------------------------
    ...
    if not df_in.empty:
        ...
        k4.metric("Total Return Global", f"${((m_tot+d_tot-c_tot)*fac):,.2f} {mon} ({gp:+.2f}%)")

        st.markdown("---")
        st.subheader("🧠 Optimización Institucional de Portafolio (Markowitz)")
        st.markdown("Calcula los pesos óptimos de tu cartera actual para maximizar el Sharpe Ratio.")
        ...
        if st.button("Calcular Frontera Eficiente"):
            with st.spinner("Calculando matriz de covarianza..."):
                ...
                try:
                    ...
                    if data.empty:
                        st.error("Error de descarga de datos por bloqueo de Yahoo Finance en la nube.")
                    else:
                    ...
                    st.error("Se necesitan al menos 2 activos válidos en cartera para calcular la covarianza o el servidor está bloqueado.")
